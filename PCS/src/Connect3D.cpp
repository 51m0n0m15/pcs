...
//				cout << "triangle " << *triangle << " created" << endl;
		...
		// DEBUG
		//int vertices[4] = { 4, 239, 383, 90 };

		//if (cellEquals(ch, vertices))
		//{
		//	cout << "tetra " << COUT_CELL(ch) << ":" << endl;
		//
		//	for (i = 0; i < 6; i++)
		...
		//	{
		//		if (triangleCreated[i])
		//			cout << "added triangle #" << i << endl;

		//		cout << "cell " << COUT_CELL(currTetra->triangle(i)->cell()) << ": ";

		//		cout << *currTetra->triangle(i) << " :";

		//		for (j = 0; j < 3; j++)
		//			cout << " " << *currTetra->triangle(i)->edge(j);
...
void writeOFFFile(string filenameStr, string extension)
{
	int i;
	string offFilenameStr = filenameStr + extension;
